...
class TomoAIMVPSupabaseUser extends BaseAuthUser {
  TomoAIMVPSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> tomoAIMVPSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TomoAIMVPSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
